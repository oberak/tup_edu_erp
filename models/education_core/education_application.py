   ...
   #overwriting 'create student' function and assign class to student
    ...
    def create_student(self,vals):
        ...
        for rec in self:
            ...
            sid=self.env['ir.sequence'].next_by_code('education.student.id')
            year = sid[len(sid)-7:len(sid)-3]
            seq = sid[len(sid)-3:len(sid)]
            student_id=year+str(rec.major_id.major_id)+seq
